...
// Build artifacts that must NEVER be copied from a template source to a
// fresh client directory. They are regenerated by `pnpm install` and
// `pnpm build` in the client repo, and copying them caused EINVAL on the
// Next.js template because `.next/standalone/` embeds the source's
// absolute path, producing a self-referential recursive copy.
export const TEMPLATE_COPY_SKIP = [
  'node_modules',
  '.next',
  '.astro',
  'dist',
...
export const shouldCopyTemplateEntry = (src: string): boolean => {
  const basename = path.basename(src)
  return !(TEMPLATE_COPY_SKIP as readonly string[]).includes(basename)
}
